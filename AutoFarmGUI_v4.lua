...
local function findCandidates()
	...
	for _, obj in ipairs(pg:GetDescendants()) do
		if obj ~= gui and not obj:IsDescendantOf(gui) then
			local score = scoreGuiObject(obj)
			if score > 0 then
				local ancestor = getOpenableAncestor(obj)
				table.insert(candidates, {
					score = score,
					obj = obj,
					open = ancestor,
					path = obj:GetFullName(),
					openPath = ancestor:GetFullName(),
				})
			end
		end
	end
...
local function openBest()
	...
	if not best then
		out.Text = "No best candidate found."
		return
	end
	...
	out.Text = "Tried opening:\n" .. best.openPath .. "\n\nIf nothing showed, click Copy and paste the candidate list here."
end
