...
local function autoVoteCard()
    ...
    -- Look for card UI and try to vote for first card
    for _, obj in ipairs(pgui:GetDescendants()) do
        if obj:IsA("TextLabel") and obj.Visible then
            if obj:IsDescendantOf(pgui:FindFirstChild("AutoFarmGUI")) then continue end
            local t = obj.Text
            -- Card titles contain "Boss Killer", "Overflowing Wealth", etc
            if string.find(t, "Boss Killer") or string.find(t, "Overflowing")
                or string.find(t, "Espionage") or string.find(t, "Reinforcement")
                or string.find(t, "Momentum") then
                -- Click the card
                local current = obj.Parent
                for d = 1, 10 do
                    if not current or current == pgui then break end
...
verBadge.Text = "v4"; verBadge.TextColor3 = Color3.fromRGB(180, 160, 255)
